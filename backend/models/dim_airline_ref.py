...
class DimAirlineRef(Base):
    ...
    __tablename__ = "Dim_Airline_Ref"

    id = Column(BigInteger, primary_key=True, index=True, autoincrement=True)
    carrier = Column(
        "CARRIER", String(10), nullable=True, comment="2-letter airline code"
    )
    airline_nation = Column(
        "Airline_Nation", String(100), nullable=True, comment="Country of airline"
    )
    airlines_name = Column(
        "Airlines_Name", String(150), nullable=True, comment="Full airline name"
    )
    created_at = Column(
    ...
    def __repr__(self):
        """Represent the DimAirlineRef model as a string"""
        return f"<DimAirlineRef(carrier='{self.carrier}', airline_nation='{self.airline_nation}', airlines_name='{self.airlines_name}')>"
