...
class FlightCleanDataStg(Base):
    """
    Model cho Flight Clean Data Staging - Bảng tạm thời cho quá trình xử lý dữ liệu

    Được sử dụng trong quá trình làm sạch và validation dữ liệu
    ...
    __tablename__ = "flight_clean_data_stg"

    id = Column(BigInteger, primary_key=True, index=True, autoincrement=True)
    flightdate = Column(String(255), nullable=True, comment="Flight date")
    flightno = Column(String(50), nullable=True, comment="Flight number")
    route = Column(String(100), nullable=True, comment="Route")
    ...
    is_invalid_flight_date = Column(
        "Is_InvalidFlightDate", Integer, nullable=True, comment="Validation flags"
    )
    is_invalid_passenger_cargo = Column(
        "Is_InvalidPassengerCargo",
        Integer,
        nullable=True,
        comment="Passenger/cargo validation",
    )
    is_invalid_route = Column(
        "Is_InvalidRoute", Integer, nullable=True, comment="Route validation"
    )
    is_invalid_actype_seat = Column(
        "Is_InvalidActypeSeat",
        Integer,
        nullable=True,
        comment="Aircraft type validation",
    )
    error_reason = Column(
        "ErrorReason", Text, nullable=True, comment="Error description"
    )
    total_errors = Column("TotalErrors", Integer, nullable=True, comment="Error count")
    ...
    def __repr__(self):
        """Represent the FlightCleanDataStg model as a string"""
        return f"<FlightCleanDataStg(flightno='{self.flightno}', total_errors={self.total_errors})>"
