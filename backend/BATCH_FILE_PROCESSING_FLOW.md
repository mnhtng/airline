...
graph TD
    ...
    B --> C[🌐 POST /upload-files API]
    C --> D[💾 Backend: Lưu vào Temp Directory]
    D --> E[🔄 Loop qua từng file]
    E --> F{📋 File đã import?}
    F -->|Yes| G[⏭️ Skip file + counter++]
    F -->|No| H[🔍 Xác định loại file MN/MB/MT]
    H --> I{✅ Loại file valid?}
    I -->|No| Z1[❌ Add error, continue]
    I -->|Yes| J[📖 Đọc & parse Excel]
    J --> K[🔧 Process theo loại file]
    K --> L{📊 Data valid?}
    L -->|No| Z2[❌ Add error, continue]
    L -->|Yes| M[💾 Lưu vào flight_raw]
    M --> N[📝 Ghi log import_log]
    N --> O[✅ Update counters]
    O --> P{🔁 Còn file?}
    P -->|Yes| E
    P -->|No| Q[💾 Commit raw data]
    Q --> R[🧹 Run SP: CleanAndProcess]
    R --> S[✔️ Run SP: Validate]
    S --> T[📊 Get Processing Summary]
    T --> U[📤 Return JSON Response]
    U --> V[🖥️ Frontend: Parse & Display]
    V --> W[🎉 Show Toast Notifications]
    W --> X[📊 Render Statistics]
```
